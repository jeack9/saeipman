 ...
 * 
 * import com.saeipman.app.ocrTest.config.OcrApi; import
 * com.saeipman.app.ocrTest.config.OcrUtil;
 * 
 ...
 * 
 * @Value("${naver.service.secretKey}") private String secretKey; private final
 * OcrApi naverApi; private final OcrUtil ocrUtil;
 * 
 * // 파일 업로드 폼을 보여주기 위한 GET 요청 핸들러 메서드
 * 
 * @GetMapping("/upload-form") public String uploadForm() throws Exception {
 * return "ocr/upload-form"; // HTML 템플릿의 이름을 반환 (upload-form.html) }
 * 
 * // 파일 업로드 및 OCR 수행을 위한 POST 요청 핸들러 메서드
 * 
 * @PostMapping("/uploadAndOcr") public String
 * uploadAndOcr(@RequestParam("file") MultipartFile file, Model model) throws
 * IOException { if (file.isEmpty()) { return "error"; // 파일이 비어있을 경우 에러를 처리하는
 * HTML 템플릿으로 이동 }
 * 
 ...
 * 
 * List<String> result = naverApi.callApi("POST", tempFile.getPath(),
 * naverSecretKey, "jpg");
 * 
 ...
 * 
 * // "도로명주소" 키워드를 기준으로 전체 주소 추출 String fullAddress = ocrUtil.mergeWords(result,
 * "도로명주소");
 * 
