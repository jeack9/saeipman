 ...
 * 
 * import jakarta.servlet.FilterChain; import
 ...
 * //시큐리티 커스텀 로그인 검증 필터
 * 
 * @RequiredArgsConstructor public class LoginFilter extends
 * UsernamePasswordAuthenticationFilter {
 * 
 ...
 * 
 * // 클라이언트 요청에서 username, password 추출 String username =
 * obtainUsername(request); String password = obtainPassword(request); // 스프링
 * 시큐리티에서 username과 password를 검증하기 위해서는 token에 담아야 함
 * UsernamePasswordAuthenticationToken authToken = new
 * UsernamePasswordAuthenticationToken(username, password, null);
 * 
 * // token에 담은 검증을 위한 AuthenticationManager로 전달 return
 ...
 * authentication) { // UserDetailsS CustomUserDetails customUserDetails =
 * (CustomUserDetails) authentication.getPrincipal();
 * 
 * String username = customUserDetails.getUsername();
 * 
 * Collection<? extends GrantedAuthority> authorities =
 * authentication.getAuthorities(); Iterator<? extends GrantedAuthority>
 * iterator = authorities.iterator(); GrantedAuthority auth = iterator.next();
 * 
